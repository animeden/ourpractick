        ...
        <div style="display: flex">
            ...
            <div style="width: 10%; height: 159px; position: fixed; display: flex; flex-direction: column; justify-content: space-around; align-items: flex-start">
                ...
                <div style="height: 35px; background: deeppink; width: 70px; display: flex; justify-content: center; align-items: center; border-top-right-radius: 20px; border-bottom-right-radius: 20px" id="sr" onclick="fup()">
                    <a class="pl-3 pt-1" href="/web/products/sort" style="color: black; text-decoration: none; display: block; height: 100%; width: 100%">Sort</a>
                </div>
                <div style="height: 35px; background: rebeccapurple; width: 70px; display: none; justify-content: center; align-items: center; border-top-right-radius: 20px; border-bottom-right-radius: 20px" id="nsr" onclick="fup()">
                    <a class="pl-2 pt-1" href="/web/products/get/list" style="color: black; text-decoration: none; display: block; height: 100%; width: 100%">Unsort</a>
                </div>
                <div style="height: 35px; background: limegreen; width: 70px; display: flex; justify-content: center; align-items: center; border-top-right-radius: 20px; border-bottom-right-radius: 20px">
                    <a class="pl-2 pt-1" href="/web/products/create" style="color: black; text-decoration: none; display: block; height: 100%; width: 100%">Create</a>
                </div>
            </div>
            ...
            <table border="3", bgcolor="#d10000", class="table table-dark">
                ...
                <#list products as product>
                    ...
                    <tr style="background: white; color: black">
                        ...
                        <td>${product.id}</td>
                        <td>${product.name}</td>
                        <td>${product.materialCode.code}</td>
                        <td>${product.preciousStoneName.name}</td>
                        <td>${product.mass}</td>
                        <td>${product.price}</td>
                        <td>${product.descriction}</td>

                        <td><a href="/web/products/edit/${product.id}"><button class="btn" style="background: #def511">Edit</button></a></td>
                        <td><a href="/web/products/delete/${product.id}" style="color: white"><button class="btn" style="background: #d60202; color: white">Delete</button></a></td>
...
<script>
    ...
    function src() {
        ...
        if (f == "http://localhost:1821/web/products/sort" || f == "https://animedenapp.herokuapp.com/web/administration/sort")
        {
            document.getElementById("sr").style.display = 'none';
            document.getElementById("nsr").style.display = 'flex';
        }
        if (f == "http://localhost:1821/web/products/get/list" || f == "https://animedenapp.herokuapp.com/web/administration/get/list")
        {
            document.getElementById("sr").style.display = 'flex';
            document.getElementById("nsr").style.display = 'none';
        }
        if(f == "http://localhost:1821/web/products/sort" || f == "http://localhost:1821/web/products/get/list")
        {
